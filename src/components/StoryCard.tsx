...
export default function StoryCard({
  ...
  const isAuthor =
    currentUserId &&
    (story.author_id === currentUserId || story.author?.id === currentUserId);
  return (
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-100">
          <div className="flex items-center space-x-2">
            {story.author_photo_url || story.author?.photoURL ? (
              <img
                src={story.author_photo_url || story.author?.photoURL}
                alt={story.author_name || story.author?.name}
                className="object-cover w-8 h-8 rounded-full"
              />
            ) : (
              <div className="flex items-center justify-center w-8 h-8 rounded-full bg-primary-100">
                <span className="text-sm font-medium text-primary-600">
                  {(story.author_name || story.author?.name || "Anonymous")
                    .charAt(0)
                    .toUpperCase()}
                </span>
              </div>
